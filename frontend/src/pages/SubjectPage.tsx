...
export default function SubjectPage() {
  ...
  if (error && !subject) {
    return (
      <div>
        ...
        <Link to="/subjects" className="text-blue hover:underline">← Back to subjects</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link to="/subjects" className="text-sm text-blue hover:underline">← All subjects</Link>
        <h2 className="mt-2 text-2xl font-bold text-ink">{subject?.name ?? "…"}</h2>
        {subject?.description && <p className="mt-1 text-dust">{subject.description}</p>}
      </div>

      <section>
        <h3 className="mb-4 text-lg font-semibold text-ink">Topics</h3>
              ...
              {draft?.id === t.id ? (
                ...
                <div className="flex items-center justify-between">
                  <Link to={`/topics/${t.id}`} className="min-w-0 flex-1">
                    <p className="font-medium text-ink hover:text-bluedark">{t.name}</p>
                    {t.description && (
      ...
      <section className="max-w-md">
        <h3 className="mb-4 text-lg font-semibold text-ink">Add a topic</h3>
